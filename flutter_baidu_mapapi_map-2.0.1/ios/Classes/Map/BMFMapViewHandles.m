...
- (NSDictionary<NSString *, NSString *> *)mapViewHandles{
    if (!_handles) {
        _handles = @{
            ...
            kMapSetCenterZoomMethod: NSStringFromClass([BMFSetMapCenterZoom class]),
...
@implementation BMFSetMapCenterZoom
...
- (void)handleMethodCall:(nonnull FlutterMethodCall *)call result:(nonnull FlutterResult)result {
    ...
    if (coordinate) {
        [_mapView setCenterCoordinate:[coordinate toCLLocationCoordinate2D] animated:[[call.arguments safeValueForKey:@"animated"] boolValue]];
        [_mapView setZoomLevel: [[call.arguments safeValueForKey:@"zoom"] intValue]];
        result(@YES);
        return;
    } else {
        result(@NO);
