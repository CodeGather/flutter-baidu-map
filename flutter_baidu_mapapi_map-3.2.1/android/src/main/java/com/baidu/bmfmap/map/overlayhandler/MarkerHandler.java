...
public class MarkerHandler extends OverlayHandler {
    ...
    public final HashMap<String, Overlay> mMarkerMap = new HashMap<>();
    public final HashMap<String, BitmapDescriptor> mMarkerBitmapMap = new HashMap<>();
    ...
    public void handlerMethodCall(MethodCall call, MethodChannel.Result result) {
        ...
        switch (methodId) {
            ...
            case Constants.MethodProtocol.MarkerProtocol.sMapUpdateMarkerMemberMethod:
                ret = updateMarkerMember((Map<String, Object>) call.arguments);
                break;
            default:
                break;
        }
     ...
     * @return
     */
    private List<LatLng> divideRouteLine(List<LatLng> routeLine, double distance) {
        // 截取后的路线点的结果集
        ...
        for (int i = 0; i < routeLine.size() - 1; i++) {
            ...
            double intercept = startPoint.latitude - slope * startPoint.longitude;
            ...
            for (double j = startPoint.latitude, k = startPoint.longitude; j > endPoint.latitude == isYReverse && k > endPoint.longitude == isXReverse; ) {
                LatLng latLng = null;
     ...
     * 循环进行移动逻辑
     */
    public void moveLooper(String id, final Map<String, Object> argument) {
        mHandler = new Handler(Looper.getMainLooper());
        final List<Map<String, Double>> coordinates = (List<Map<String, Double>>) argument.get("coordinates");
        final Marker mMoveMarker = (Marker) mMarkerMap.get(id);
        List<LatLng> latLngList = FlutterDataConveter.mapToLatlngs(coordinates);
        if (latLngList == null || latLngList.size() == 0) {return;}
        latLngs = divideRouteLine(latLngList, 0.00009);
...
//                                mMoveMarker.setRotate((float) getAngle(startPoint, endPoint));
                                ...
                                hashMap.put("latitude", startPoint.latitude);
                                hashMap.put("longitude", startPoint.longitude);
                                argument.put("value", hashMap);
                                argument.put("member", "position");
                                updateMarkerMember(argument);
                            }
                        });
