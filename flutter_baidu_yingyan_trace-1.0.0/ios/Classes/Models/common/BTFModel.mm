...
@implementation BTFModel

+ (nonnull instancetype)btf_modelWith:(nonnull NSDictionary *)dic {
    // 初始化
    ...
    for (unsigned int i = 0; i < outCount; i++) {
        ...
        // 检测忽略的propertyName
        NSArray *ignoreArray;
        if ([[obj class] respondsToSelector:@selector(btf_setupIgnorePropertyNames)]) {
           ignoreArray = [[obj class] btf_setupIgnorePropertyNames].copy;
        }
        if ([ignoreArray containsObject:propertyName]) continue;
        
         // 检测有没有需要替换的propertyName
        NSDictionary *replaceDic;
         if ([[obj class] respondsToSelector:@selector(btf_setupReplacedKeyFromPropertyName)]) {
             replaceDic = [[obj class] btf_setupReplacedKeyFromPropertyName];
         }
        ...
        if ([value isKindOfClass:[NSArray class]]) {
            if ([[obj class] respondsToSelector:@selector(btf_setupObjectClassInArray)]) {

                NSString *type = [[obj class] btf_setupObjectClassInArray][propertyName];
                Class cls = objc_getClass(type.UTF8String);
                if (cls && ![type hasPrefix:@"NS"]) { //自定义model
                    NSMutableArray *models = [NSMutableArray array];
                    for (NSDictionary *_dic in value) {
                        id model = [cls btf_modelWith:_dic];
...
//                          id model = [[objChild class] btf]
//                        }
//                    }
                }
            } else {
                //TODO:未指定数组model类型
                NSLog(@"dic -> model %@未指定数组model类型", propertyName);
            }
        } else if ([value isKindOfClass:[NSDictionary class]]) {

            if (![propertyType hasPrefix:@"NS"]) { //propertyType = model;
                Class cls = objc_getClass(propertyType.UTF8String);
                id obj = [cls btf_modelWith:(NSDictionary *)value];
                value = obj ? obj : nil;
            }
            if (value) {
                [obj setValue:value forKey:propertyName];
...
+ (NSDictionary *)btf_dicFrom:(id)model{
    // key = propertyName
    ...
    for (unsigned int i = 0; i < count; i++) {
        ...
        NSDictionary *replaceDic;
        if ([[model class] respondsToSelector:@selector(btf_setupReplacedKeyFromPropertyName)]) {
            replaceDic = [[model class] btf_setupReplacedKeyFromPropertyName];
        }
        // 替换propertyName
        ...
        // value为对象
        if ([value isKindOfClass:[NSArray class]]) {
            if ([[model class] respondsToSelector:@selector(btf_setupObjectClassInArray)]) {
                // 获取数组model对应的类型
                NSString *type = [[model class] btf_setupObjectClassInArray][propertyName];
                Class cls = objc_getClass(type.UTF8String);

                if (cls && ![type hasPrefix:@"NS"]) { //自定义model
                    NSMutableArray *models = [NSMutableArray array];
                    for (id obj in value) {
                        id m = [cls btf_dicFrom:obj];
                        if (m) {
                            [models addObject:m];
                        }
//                        !m ? : [models addObject:m];
                    ...
                    NSMutableArray *models = [NSMutableArray array];
                    for (NSArray *model in value) {
                        NSMutableArray *modelC = [NSMutableArray array];
                        for (id obj in model) {
                            id m = [[obj class] btf_dicFrom:obj];
                            if (m) {
                                [modelC addObject:m];
                            }
//                            !m ? : [modelC addObject:m];
            ...
            for (NSString *key in [(NSDictionary *)value allKeys]) {
                id obj = value[key];
                Class cls = [obj class];
                // key:model(自定义model中不要包含NS)
                if (cls && ![NSStringFromClass(cls) containsString:@"NS"]) {
                    id m = [cls btf_dicFrom:obj];
                    [vDic setObject:m forKey:key];
                } else if ([NSStringFromClass(cls) containsString:@"NS"]) { //key:NSObject
...
//            !vDic ? : [dic setObject:vDic forKey:propertyName];
            ...
            if (model && model !=[NSNull null]) {
                id m = [[model class] btf_dicFrom:value];
                if (m) {
...
- (NSDictionary *)btf_toDictionary{
    return [[self class] btf_dicFrom:self];
}
@end
