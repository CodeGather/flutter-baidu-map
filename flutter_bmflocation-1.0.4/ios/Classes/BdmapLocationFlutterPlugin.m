...
#import "BdmapLocationFlutterPlugin.h"
#import "BMKLocationkit/BMKLocationComponent.h"
#import "BdmapFlutterStreamManager.h"
#import "BaiduMapAPI_Utils/BMKUtilsComponent.h"

@interface LocationFlutterPlugin()<BMKLocationManagerDelegate>
...
@implementation LocationFlutterPlugin

+ (void)registerWithRegistrar:(NSObject<FlutterPluginRegistrar>*)registrar {
    
    FlutterMethodChannel* channel = [FlutterMethodChannel
                                     methodChannelWithName:@"bdmap_location_flutter_plugin"
                                     binaryMessenger:[registrar messenger]];
    
    LocationFlutterPlugin* instance = [[LocationFlutterPlugin alloc] init];
    
    [registrar addMethodCallDelegate:instance channel:channel];
    
    FlutterEventChannel *eventChanel = [FlutterEventChannel eventChannelWithName:@"bdmap_location_flutter_plugin_stream" binaryMessenger:[registrar messenger]];
...
//        NSLog((@"\n[bdmap_loc_flutter_plugin:%s]"), "stopLocation...");
    ...
    } else if ([@"getDistance" isEqualToString:call.method] ) { // 设置定位参数
      //导入工具功能包BaiduMapAPI_Utils.framework
      NSDictionary *current = call.arguments;
      NSDictionary *startMap = current[@"start"];
      NSDictionary *endMap = current[@"end"];
      CLLocationCoordinate2D startCoor = CLLocationCoordinate2DMake([startMap[@"latitude"] doubleValue], [startMap[@"longitude"] doubleValue]);
      CLLocationCoordinate2D endCoor = CLLocationCoordinate2DMake([endMap[@"latitude"] doubleValue], [endMap[@"longitude"] doubleValue]);
      BMKMapPoint startPoint = BMKMapPointForCoordinate(startCoor);
      BMKMapPoint endPoint = BMKMapPointForCoordinate(endCoor);
      CLLocationDistance distance = BMKMetersBetweenMapPoints(startPoint, endPoint);
      result(@(distance));
...
//            NSLog(@"最小更新距离值:%f", [args[@"distanceFilter"] doubleValue]);
        ...
        if ([[args allKeys] containsObject:@"BMKLocationCoordinateType"]) {
            [self.locManager setCoordinateType:[ self getCoordinateType: args[@"BMKLocationCoordinateType"]]];
        }
        
        // 设置应用位置类型
        if ([[args allKeys] containsObject:@"activityType"]) {
            [self.locManager setActivityType:[ self getActivityType: args[@"activityType"]]];
        }
        
        // 设置是否需要返回新版本rgc信息
        if ([[args allKeys] containsObject:@"isNeedNewVersionRgc"]) {
            if ((bool)args[@"isNeedNewVersionRgc"]) {
//                NSLog(@"需要返回新版本rgc信息");
                self.locManager.isNeedNewVersionReGeocode = YES;
            } else {
//                NSLog(@"不需要返回新版本rgc信息");
...
//                  NSLog(@"设置不允许后台定位");
      ...
      // 设置是否返回地理信息
      if ([[args allKeys] containsObject:@"isNeedAddres"]) {
        [self.locManager setLocatingWithReGeocode: [args[@"isNeedAddres"] boolValue]];
      }
